...
- 默认目标是“带用户完成配置并告诉用户如何启动”，不是默认替用户把服务直接启动起来
...
- 不允许为了“先跑起来”而擅自伪造模型配置
- 不允许在用户没有提供真实 `model_provider`、`model`、`base_url`、`api_key` 时，自行编造占位值并当作可用配置写入
- 如果用户暂时没有提供模型信息，可以明确告诉用户“环境已配到模型这一步，服务启动前还差真实模型配置”，但不能假装已经完成
...
- 小助手可以提供模板和示例格式
- 小助手可以帮助用户把真实值整理成最终配置
- 但小助手不能自己臆造一个模型名、接口地址或 API Key 填进去
- 只要模型四元组不是用户亲自提供或仓库里已经存在的真实可用值，就必须视为“尚未完成配置”
...
- 不能为了让服务先启动，就自己编造占位模型配置
- 不能把假模型、假 `base_url`、假 `api_key` 写进去后，对用户说“已经部署好了”
- 如果用户还没提供模型信息，正确结论应该是：“runtime-service 模板已经准备好，但真实模型配置还没完成，因此你现在还不能进行真实对话调用”
...
如果用户暂时不想提供模型配置，你必须明确告诉他当前停留在哪一步，例如：

```text
现在 runtime-service 的模板文件可以先准备好，但真实模型配置还没有提供。

这意味着：
- 我们可以先把数据库、platform-api、两个前端的准备工作继续梳理清楚
- 但在你提供真实模型四元组之前，我不能替你编造模型配置，也不能把 runtime-service 说成“已经完整可用”
```
...
### 先告诉用户：这里有两种启动方式，但默认由用户自己执行

这一节的核心要求不是“助手替用户启动”，而是：

- 小助手把启动顺序、启动命令、健康检查方式讲清楚
- 小助手明确告诉用户每条命令应该在哪个目录执行
- 默认由用户自己在本地终端执行
- 只有用户明确要求“小助手代为执行启动”时，才进入代执行模式
...
我会把两种启动方式、启动顺序、健康检查和停止方式都告诉你，你按这些命令自己执行就可以。

如果你之后希望我代你执行启动命令，你再明确告诉我，我再继续。
```
...
```text
如果你现在更希望快速把整个本地环境拉起来，可以在仓库根目录自己执行 `scripts/dev-up.sh`。
...
当 `platform-api` 使用推荐最小本地配置，且未额外修改：
...
```text
如果你按当前仓库推荐的本地最小配置启动 `platform-api`，并且没有改过 bootstrap 登录配置，那么平台默认登录账号是：
- username: admin
- password: admin123456

如果你后面改过 `BOOTSTRAP_ADMIN_USERNAME` 或 `BOOTSTRAP_ADMIN_PASSWORD`，那就以你自己改后的值为准。
```

### 这一节必须明确告诉用户“应该如何启动”

在步骤全部完成后，你必须给出一段可以直接照着执行的启动摘要，而且默认表述为“请用户自己执行”：

```text
如果你现在要自己启动，推荐第一次按下面顺序分别在对应目录执行：
...
如果你现在要自己重新启动，推荐顺序就是：
1. 先在 `apps/runtime-service` 执行 `uv run langgraph dev --config graph_src_v2/langgraph.json --port 8123 --no-browser`
2. 再在 `apps/platform-api` 执行 `uv run uvicorn main:app --host 0.0.0.0 --port 2024`
3. 再在 `apps/platform-web` 执行 `pnpm dev`
4. 需要时再在 `apps/runtime-web` 执行 `PORT=3001 pnpm dev`

如果你只想快速整套启动，也可以自己在仓库根目录执行：
- `scripts/dev-up.sh`
...
- 一上来把全部部署命令直接甩给用户
- 遇到版本不一致时装作没看见
- 用户没有提供真实模型信息时，擅自编造模型配置
- 把占位模型配置说成真实可用部署结果
- 用户没有明确要求时，默认替用户直接执行启动命令
