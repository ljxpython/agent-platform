...
async def list_user(
    ...
    data = [
        await obj.to_dict(m2m=True, exclude_fields=["password"]) for obj in user_objs
    ]
    for item in data:
        dept_id = item.pop("dept_id", None)
        item["dept"] = (
            await (await dept_controller.get(id=dept_id)).to_dict() if dept_id else {}
        )
...
async def create_user(
    ...
    if user:
        return Fail(
            code=400, msg="The user with this email already exists in the system."
        )
