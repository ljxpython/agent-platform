...
使用方式（需要在 apps/runtime-service 目录执行，确保可 import graph_src_v2）：
- `python -m graph_src_v2.devtools.dump_multimodal_fixtures`
- `python -m graph_src_v2.devtools.dump_multimodal_fixtures /path/to/dir_or_file --pdf-preview-chars 200`
"""
...
from graph_src_v2.devtools.multimodal_frontend_compat import (
    decode_block_bytes,
    file_path_to_frontend_content_block,
    image_block_to_data_url,
)
from graph_src_v2.middlewares.multimodal import _extract_pdf_text
...
def _iter_default_fixture_paths() -> list[Path]:
    # 默认 fixtures 目录：graph_src_v2/test_data
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "可选：fixture 文件路径或目录。"
            "若不传，则默认使用 graph_src_v2/test_data 下的 fixtures。"
        ),
    )
    parser.add_argument(
        "--pdf-preview-chars",
    ...
    if not paths:
        print(
            _format_for_console(
                "No fixtures found. Provide paths or add files under graph_src_v2/test_data.",
