# Platform API V2 循环导入治理清单

这份清单只服务一件事：

> 把 `platform-api-v2` 当前已经暴露出来的循环导入问题彻底收掉，并冻结成后续不得回退的模块规则。
...
- `uv run --project apps/platform-api-v2 pytest apps/platform-api-v2/tests/test_phase6_normalization_regressions.py -q`
...
这条任务完成后，`platform-api-v2` 必须遵守下面的冻结规则：
...
- `uv run --project apps/platform-api-v2 pytest apps/platform-api-v2/tests/test_phase6_normalization_regressions.py -q`
- `uv run --project apps/platform-api-v2 python -m compileall app`
...
这不是在修一个 pytest 小毛病，而是在修 `platform-api-v2` 当前模块边界不稳的问题。
