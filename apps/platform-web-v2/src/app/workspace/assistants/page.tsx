...
import { listAssistantsPage, type ManagementAssistant } from "@/lib/management-api/assistants";
import { useWorkspaceContext } from "@/providers/WorkspaceProvider";

function getAssistantStatusVariant(status: string): "success" | "warning" | "neutral" {
  if (status === "active") {
    return "success";
  }
  if (status === "disabled") {
...
function getSyncStatusVariant(status: string): "success" | "warning" | "danger" | "neutral" {
...
export default function AssistantsPage() {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      setError(null);
      try {
        const payload = await listAssistantsPage(projectId, { limit: pageSize, offset, query });
        if (cancelled) {
          return;
        }
        ...
        if (!cancelled) {
          setError(
            fetchError instanceof Error ? fetchError.message : "Failed to load assistants",
          );
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <PlatformPage>
      <PageHeader
        description="助手页依赖当前项目上下文，所以这里直接接入了 workspace provider 的 project 选择。后续图谱、线程和 runtime 也会沿用这一套上下文。"
        eyebrow="Workspace"
        title="Assistants"
      />
      ...
      {!projectId ? (
        ...
        >
          {loading ? <div className="text-sm text-[var(--muted-foreground)]">Loading assistants...</div> : null}
                ...
                <table className="min-w-[980px] w-full border-collapse">
                  <thead>
                    <tr className="border-b" style={{ borderColor: "var(--border)" }}>
                      {["Assistant", "Graph", "Status", "Sync", "Runtime", "Updated"].map((label) => (
                        <th
                          key={label}
